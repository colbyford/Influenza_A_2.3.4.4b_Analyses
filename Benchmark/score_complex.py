...
def score_complex(pdb_path: str, ligand_chain: str, n_cores: int = os.cpu_count()) -> dict:
    ...
    mkdir_path = Path("haddock_score_temp")
    mkdir_path.mkdir(exist_ok=True)
    ligand_pdb = Path("haddock_score_temp/ligand.pdb").resolve()
    ...
    try:
        subprocess.run(
            ...
            capture_output=True, text=True, check=True, cwd=mkdir_path
        )
    ...
    [topoaa]
    autohis = true
    ligand_param_fname = "{mkdir_path}/ligand.acpype/ligand_CNS.par"
    ligand_top_fname = "{mkdir_path}/ligand.acpype/ligand_CNS.top"
    [mdscoring]
    ligand_param_fname = "{mkdir_path}/ligand.acpype/ligand_CNS.par"
    ligand_top_fname = "{mkdir_path}/ligand.acpype/ligand_CNS.top"
    nemsteps = 1
    ...
    try:
        ...
        capri_file = mkdir_path / "run0/analysis/1_mdscoring_analysis/capri_ss.tsv"
