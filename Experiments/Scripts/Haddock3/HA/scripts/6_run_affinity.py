...
for index, row in experiments.iterrows():
    ...
    capri_file_path = f'2_Docking/{value}/output/12_caprieval/capri_ss.tsv'
...
for index, row in experiments.iterrows():
    ...
    combination_value = row['combination']
    ...
    prodigy_output = os.popen(f'prodigy_lig -c A,I I:SIA -i {uncompressed_pdb_path}').read()
    prodigy_output_lines = prodigy_output.split('\n')
    
    # Extract the second and third columns from the output
    if len(prodigy_output_lines) > 1:
        columns = prodigy_output_lines[1].split()
        predicted_binding_affinity = float(columns[1])
        dg_score = float(columns[2])

        ## Add values to DataFrame
        experiments.loc[index, 'prodigy_deltaG_kcalpermol'] = predicted_binding_affinity
        experiments.loc[index, 'prodigy_dg_score'] = dg_score
